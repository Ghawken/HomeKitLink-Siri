...
###
    ...
    def deviceUpdated(self, original_device, updated_device):
        ...
        try:
            ...
            if checkindex != None:
                ...
                elif str(updateddevice_subtype) in ("LeakSensor", "OccupancySensor", "SmokeSensor", "ContactSensor", "CarbonMonoxideSensor", "MotionSensor", "CarbonDioxideSensor"):  ## example of one way sesnor
                    if type(original_device) == indigo.SensorDevice:
                        sensortouse = self.device_list_internal[checkindex]["devicesensor"]
                        if sensortouse != "sensorValue":
                            if sensortouse != "":
    ...
    def Plugin_getter_callback(self, accessoryself, statetoGet):
        ...
        try:
            ...
            if statetoGet in ("temperature", "SensorLeak", "humidity", "LightLevel", "sensorOccupancy", "motionSensor","sensorSmoke", "SensorCarbonDioxide", "SensorCarbonMonoxide", "sensorContactSensor"):  ## this could actually be a number of options..
    ...
    def Menu_mDNS_troubleshoot(self, *args, **kwargs):
        ...
        try:
            ...
            # Note: lsof -i UDP:5353 requires root — listed in the manual Terminal commands section (Step 13).
            ...
            self.logger.info("")
            self.logger.info("  sudo log stream --predicate 'process == \"mDNSResponder\" AND messageType == error AND NOT composedMessage CONTAINS \"NSEC\"'")
            self.logger.info("      Stream mDNSResponder errors only, filtering out noisy NSEC messages.")
            self.logger.info("      NSEC errors are common and usually harmless — this shows the errors that matter.")
            self.logger.info("")
            self.logger.info("  sudo log show --predicate 'process == \"mDNSResponder\"' --info --style compact --last 5m")
            self.logger.info("      Show recent mDNSResponder log entries from the last 5 minutes (adjust as needed).")
            self.logger.info("      Useful for reviewing what happened around a specific failure without live streaming.")
            self.logger.info("")
            self.logger.info("  sudo log show --predicate 'process == \"mDNSResponder\" AND composedMessage CONTAINS \"_hap\"' --info --last 10m")
            self.logger.info("      Show only HAP-related mDNSResponder log entries from the last 10 minutes.")
            self.logger.info("      Filters to _hap._tcp service activity (registrations, queries, responses).")
            self.logger.info("")
            self.logger.info("  sudo dscacheutil -flushcache && sudo killall -HUP mDNSResponder")
            self.logger.info("      Flush the DNS cache and send HUP to mDNSResponder to clear its cache.")
            self.logger.info("      Can resolve stale DNS/mDNS entries. macOS restarts mDNSResponder automatically.")
            self.logger.info("")
            self.logger.info("  sudo pfctl -s rules 2>/dev/null | head -20")
            self.logger.info("      Show the first 20 active packet-filter (pf) firewall rules.")
            self.logger.info("      Check for rules that might be blocking mDNS (UDP 5353) or HAP server traffic.")
            self.logger.info("      (stderr suppressed because pfctl prints 'No ALTQ support' warnings on most systems.)")
